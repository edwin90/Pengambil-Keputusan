...
if(isset($_POST['id']) && strlen($_POST['id'])){
	$id = $_POST['id'];
	$data_sli = @mysql_fetch_array(mysql_query("SELECT * FROM tb_solusi WHERE kd_solusi='$id'"));
	if($id!='0'){ 
		$kode = $data_sli['kd_solusi'];
		$nama = $data_sli['nm_solusi'];
	}else{
		$kode ="";
		$nama ="";
	}
...
<div id="form-data-entry-header">
	<h2><span class="judul"></span> Data Solusi Kerusakan<a class="icon-tutup ico-remove" title="Tutup"></a></h2>
</div>
<div id="form-data-entry-body">
	<table cellspacing="0" cellpadding="0">
		<tr>
			<td width="25%" align="right">Kode</td>
			<td><input name="kode" size="5" maxlength="4" value="<?php echo $kode; ?>"required autofocus></input></td>
		</tr>
		<tr>
			<td valign="top" align="right">Nama Solusi</td>
	...
	$("#btn-simpan").click(function(){
		...
		var v_nama = $('textarea[name=nama]').val();
			if (v_kode==""){
				...
				alert("Nama solusi masih kosong !");
				$('textarea[name=nama]').focus();
				return false;
			}else{
				$.post(url_proses, {kd_solusi: v_kode, nm_solusi: v_nama, id: kode} ,function(){
					$("#backdrop, #form-data-entry").fadeOut("fast");
					$('#form-data-entry').html("");
					$('body').load(show(hal));
					return false;
